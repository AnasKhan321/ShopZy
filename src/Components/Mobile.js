...
const Mobile = (props) => {
    ...
    const fetchdata = async()=>{
        ...
        const res =  await fetch(`http://localhost:3000/api/product/categoryproduct/Mobile`)
    ...
    return (
        <div className="container mx-auto my-4 min-h-screen  ">
                {data.length !==0 &&   <h1    className="text-center font-bold text-2xl my-6" > Top SmartPhone </h1>  }
                <div className="cards flex flex-wrap justify-between  ">
                    {data.length == 0 && <h1 className=" mx-auto  text-center text-2xl "> New Stock of SmartPhone will be available Soon ! </h1>}
                    {data.map((e)=>{
                    return(
