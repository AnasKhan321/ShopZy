...
const OrderDetail = () => {
    ...
    const fetchData = async()=>{
        ...
        let res = await fetch(`http://localhost:3000/api/order/orderbyid/${id}`); 
...
<div class="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">
  ...
  <div class="flex justify-start item-start space-y-2 flex-col">
    <h1 class="text-3xl  lg:text-4xl font-semibold leading-7 lg:leading-9 text-gray-800">Order #{Order._id}</h1>
    <p class="text-base  font-medium leading-6 ">  { StringToDate( Order.Orderat)} </p>
  </div>
  <div class="mt-10 flex flex-col xl:flex-row jusitfy-center items-stretch w-full xl:space-x-8 space-y-4 md:space-y-6 xl:space-y-0">
    <div class="flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8">
      <div class="flex flex-col justify-start items-start dark:bg-gray-800 bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full">
        <p class="text-lg md:text-xl dark:text-white font-semibold leading-6 xl:leading-5 text-gray-800"> Your Order Details </p>

        {Order && Order.items &&   Object.keys(Order?.items)?.map((e)=>{
            return( <div class="mt-4 md:mt-6 flex flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full">
            <div class="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full pb-8 space-y-4 md:space-y-0">
              <div class="w-full flex flex-col justify-start items-start space-y-8">
                <h3 class="text-xl dark:text-white xl:text-2xl font-semibold leading-6 text-gray-800">{Order.items[e].name}</h3>
              ...
              <div class="flex justify-between space-x-8 items-start w-full">
                <p class="text-base dark:text-white xl:text-lg leading-6">{ formatPriceWithThousandSeparator( Order.items[e].price)} </p>
                <p class="text-base dark:text-white xl:text-lg leading-6 text-gray-800">{Order.items[e].qty }</p>
                <p class="text-base dark:text-white xl:text-lg font-semibold leading-6 text-gray-800">{ formatPriceWithThousandSeparator( parseInt( Order.items[e].price) * parseInt(Order.items[e].qty) ) }</p>
              </div>
            </div>
          </div>)
        })}
      ...
      <div class="flex justify-center flex-col md:flex-row flex-col items-stretch w-full space-y-4 md:space-y-0 md:space-x-6 xl:space-x-8">
        <div class="flex flex-col px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 dark:bg-gray-800 space-y-6">
          ...
          <div class="flex justify-center items-center w-full space-y-4 flex-col border-gray-200 border-b pb-4">
            <div class="flex justify-between w-full">
              ...
              <p class="text-base dark:text-gray-300 leading-4 text-gray-600">{ formatPriceWithThousandSeparator( Order.bill)}</p>
            </div>
          ...
          <div class="flex justify-between items-center w-full">
            ...
            <p class="text-base dark:text-gray-300 font-semibold leading-4 text-gray-600">{ formatPriceWithThousandSeparator( Order.bill+50)}</p>
          </div>
        </div>
        <div class="flex flex-col justify-center px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 dark:bg-gray-800 space-y-6">
          ...
          <div class="flex justify-between items-start w-full">
            ...
            <p class="text-lg font-semibold leading-6 dark:text-white text-gray-800">50</p>
          </div>
      ...
      <div class="flex flex-col md:flex-row xl:flex-col justify-start items-stretch h-full w-full md:space-x-6 lg:space-x-8 xl:space-x-0">
        <div class="flex flex-col justify-start items-start flex-shrink-0">
          <div class="flex justify-center w-full md:justify-start items-center space-x-4 py-8 border-b border-gray-200">
            ...
            <div class="flex justify-start items-start flex-col space-y-2">
              <p class="text-base dark:text-white font-semibold leading-4 text-left text-gray-800">{Order.name}</p>
        ...
        <div class="flex justify-between xl:h-full items-stretch w-full flex-col mt-6 md:mt-0">
          <div class="flex justify-center md:justify-start xl:flex-col flex-col md:space-x-6 lg:space-x-8 xl:space-x-0 space-y-4 xl:space-y-12 md:space-y-0 md:flex-row items-center md:items-start">
            <div class="flex justify-center md:justify-start items-center md:items-start flex-col space-y-4 xl:mt-8">
              ...
              <p class="w-48 lg:w-full dark:text-gray-300 xl:w-48 text-center md:text-left text-sm leading-5 text-gray-600">{Order.state} {Order.city} </p>
            </div>
            <div class="flex justify-center md:justify-start items-center md:items-start flex-col space-y-4">
              ...
              <p class="w-48 lg:w-full dark:text-gray-300 xl:w-48 text-center md:text-left text-sm leading-5 text-gray-600">{Order.landmark} {Order.address}</p>
            </div>
          </div>
